...
> Agregare dos commits más a la rama `new-feature` para que el ejercicio se entienda mejor
...
>- Después, al hacer `git checkout main` y `git merge feature` el historial de commits será lineal, tanto para `main`, como para `feature`. Ambos tendrán la misma historial lineal de commits.
...
> [!important] Mi rama principal local se llama `master`
